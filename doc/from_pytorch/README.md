...
原来的任务 01—11 是按开发顺序写的。这里按理解所需的前置知识重排：先理解为什么有这些
模块，再看它们怎么实现。每节都有 PyTorch 对照、项目代码位置、调用点、实验和参考答案。
...
打开几十个网页。读完前三节后，再进入原来的任务 09、10、11。
...
旧任务文档中的历史性能表保留当时的数据。当前实现状态看这套入门文档、最新任务和
